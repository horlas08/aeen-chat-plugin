...
class Register extends WWO {
	...
	public function register_otp() {
		...
		// Check if AeenWoo API is configured
        if ( ! AeenWoo_API::is_configured() ) {
			wp_send_json_error(
				array(
					'message' => '<li class="ware-notice danger"><i class="bi bi-exclamation-triangle-fill"></i>' . esc_html__( 'Plugin settings are missing.', 'ware' ) . '</li>',
				)
			);
			exit();
		}
	...
	public function enqueue() {
		if ( ! is_user_logged_in() ) {
			wp_enqueue_script( 'ware-register', AEENWOO_URL . 'assets/js/my-account-register.js', array( 'jquery' ), '1.0.0', true );
		}
	}
}
?>
