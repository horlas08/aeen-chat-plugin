	...
	public function login_otp() {
		...
		if ( count( $user_exist ) > 0 && strlen( $phone ) > 10 ) {
			...
			if ( ! AeenWoo_API::is_configured() ) {
				wp_send_json_error(
					array(
						'message' => '<li class="ware-notice danger"><i class="bi bi-exclamation-triangle-fill"></i>' . esc_html__( 'Plugin settings are missing. Please check your settings.', 'ware' ) . '</li>',
					)
				);
				exit;
			}
            
            // Get message template from wwo_settings if exists, or use default. 
            // Ideally we should move this template setting to new settings page, but for now fallback is fine.
            $settings = get_option( 'wwo_settings' );
            ...
            $response = AeenWoo_API::send_message( $phone, $message );

			if ( 'success' !== $response['status'] ) {
				wp_send_json_error(
					array(
						'message' => '<li class="ware-notice danger"><i class="bi bi-exclamation-triangle-fill"></i>' . esc_html__( 'Failed to send passkey. ', 'ware' ) . $response['message'] . '</li>',
					)
				);
			} else {
...
public function login() {
	...
	public function enqueue() {
		...
		if ( is_rtl() ) {
			wp_enqueue_style( 'custom-my-account-rtl', AEENWOO_URL . 'assets/css/my-account-rtl.css', array(), '1.0.0' );
		}

		wp_enqueue_style( 'custom-my-account', AEENWOO_URL . 'assets/css/my-account.css', array(), '1.0.0' );
		wp_enqueue_script( 'custom-my-account', AEENWOO_URL . 'assets/js/my-account.js', array( 'jquery' ), '1.0.0', true );
		wp_enqueue_script( 'ware-login', AEENWOO_URL . 'assets/js/my-account-login.js', array( 'jquery' ), '1.0.0', true );
