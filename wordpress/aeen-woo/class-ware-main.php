...
class ware_Main {
	...
	public function ware_admin_menu() {
		...
		add_submenu_page(
			...
			'manage_options',
			'ware-settings',
			array( new AeenWoo_Settings(), 'render_settings_page' )
		);
			add_submenu_page(
				'ware',
...
public function ware_wa_send_msg( $config, $phone, $msg, $img, $resend, $notification_type = '', $order_id = null ) {
        ...
        // Use AeenWoo API
        $response = AeenWoo_API::send_message( $phone, $msg, $img );

		$plugin_status = ( 'success' === $response['status'] ) ? 'Success' : 'Error';
		...
		// Log the result with additional data
		$this->log->add(
			'waresend',
			...
			'<td><a href="#" class="plugin-status-link status-' . esc_attr( $plugin_status_class ) . '" data-popup="' . esc_attr( json_encode( $result ) ) . '">' . esc_html( $plugin_status ) . '</a></td>' .
			'<td><button type="button" class="button log-resend" data-phone="' . esc_html( $phone ) . '" data-message="' . esc_html( $msg ) . '" data-img="' . esc_html( $img ) . '" data-notification-type="' . esc_html( $notification_type ) . '">Resend WhatsApp</button></td>' .
			'</tr>'
		);
