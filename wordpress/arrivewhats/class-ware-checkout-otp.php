...
class ware_checkout_otp {
	...
	public function send_otp_ajax_handler() {
		...
		if ( empty( $instance_id ) || empty( $access_token ) ) {
			error_log( esc_html__( 'Instance ID or Access Token is missing.', 'ware' ) );
			wp_send_json_error( esc_html__( 'Instance ID or Access Token is missing.', 'ware' ) );
			wp_die();
		}
		...
		$urlwa = 'https://app.arrivewhats.com/api/send?number=' . $phone_number . '&type=text&message=' . $message . '&instance_id=' . $instance_id . '&access_token=' . $access_token;
        $response = wp_remote_post( $urlwa , $args);

		if ( is_wp_error( $response ) ) {
			$error_message = $response->get_error_message();
			error_log( esc_html__( 'Failed to send OTP via WhatsApp. Error: ', 'ware' ) . $error_message );
			wp_send_json_error( esc_html__( 'Failed to send OTP via WhatsApp. Error: ', 'ware' ) . $error_message );
		} else {
			...
			if ( $response_code != 200 ) {
				wp_send_json_error( esc_html__( 'Failed to send OTP via WhatsApp. Response: ', 'ware' ) . $response_body );
			} else {
