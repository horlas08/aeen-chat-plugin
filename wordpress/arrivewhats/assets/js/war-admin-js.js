...
jQuery(function ($) {
    $("textarea.war-emoji").emojioneArea({
...
function initializePlaceholderDropdown(containerClass, placeholders, promptText) {
    ...
    $('.placeholder-dropdown').change(function () {
        ...
        var textarea = $(this).closest('.notification').find('textarea.war-emoji');
...
$(document).ready(function() {
    ...
    if ($("#war_test_number").length) {
    var iti_war = window.intlTelInput(document.querySelector("#war_test_number"), {
        initialCountry: "auto",
    ...
    $('#war_test_number').on('blur', function () {
        $(this).val(iti_war.getNumber().replace('+', ''));
    });

    window.iti_war = iti_war;
}
...
if ($("#admin_number").length) {
    ...
    $('.war-tab-wrapper .nav-tab-wrapper').each(function () {
    ...
    $('.war-panel-footer input[type=submit]').click(function (event) {
    ...
    $('.war-add-item').click(function () {
        var new_li = Date.now() / 1000 | 0;
        var ul = $('#war-sortable-items');
        var li = `<li id="war_item_${new_li}">
                    <header>
                        <i class="dashicons-before dashicons-arrow-down-alt2" aria-hidden="true"></i> New Rule
                    </header>
                    <div class="war-item-body">
                        <div class="war-body-left">
                            <p class="war-match">
                                <label for="keyword-match-${new_li}">Keyword Match</label>
                                <select id="keyword-match-${new_li}" class="widefat" name="war_autoresponders[items][${new_li}][item_match]">
                            ...
                            <p class="war-keyword">
                                ...
                                <input type="text" id="chat-keyword-${new_li}" class="widefat" name="war_autoresponders[items][${new_li}][item_keyword]">
                            </p>
                        </div>
                        <div class="war-body-right">
                            <p>
                                <label for="autoresponder-reply-${new_li}">Autoresponder Reply</label>
                                <textarea rows="5" id="autoresponder-reply-${new_li}" class="widefat" name="war_autoresponders[items][${new_li}][item_reply]"></textarea>
                            </p>
                            <p class="war-upload-img">
                                <input type="button" name="upload-btn" class="upload-btn button-secondary" data-id="item-img-${new_li}" value="Upload Image">
                                <input type="text" name="war_autoresponders[items][${new_li}][item_img]" placeholder="Image URL (Max 1 MB)" class="image_url regular-text item-img-${new_li}">
                            </p>
                        </div>
                        <div class="war-item-controls">
                            <a href="#" class="war-remove-item">Delete</a>
                        </div>
                    </div>
                </li>`;
        ul.prepend(li);
        $('#war-sortable-items').accordion("refresh");
        war_action('#war-sortable-items');
        return false;
    });
    ...
    function war_action(el) {
        var items_sort = $(el).sortable('serialize', { key: 'sort' });
        $('#war-items-order').val(items_sort);
        $('.war-remove-item').click(function () {
            $('#war-sortable-items').accordion('option', { active: false });
            $(this).parents('li').remove();
            return false;
        });
    }

    $('.war-tab-wrapper').on("click", '.upload-btn', function (e) {
        e.preventDefault();
    ...
    $("#war_broadcast_target").change(function () {
        if ($(this).val() === "custom") {
